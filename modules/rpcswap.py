...
class RpcSwap(Approve):
    ...
    def remove_liquidity(self, token_to_remove, retry=0):
        try:
            self.log.info(f'Remove Liquidity {token_to_remove["name"]} token on RPCswap')

            token_contract = self.web3.eth.contract(address=token_to_remove['lp rpc'], abi=self.token_remove_abi)
            token_balance = token_contract.functions.balanceOf(self.address_wallet).call()
            if token_balance == 0:
            ...
            if allowance < 10000 * 10 ** decimal:
                self.log.info('Нужен аппрув, делаю')
                self.approve(token_to_remove['lp rpc'], self.address)
                time.sleep(60)
            ...
            if tx_receipt.status == 1:
                ...
                self.log.error('Транзакция сфейлилась, пытаюсь еще раз')
                # if TgBot.TG_BOT_SEND is True:
                #     TgBot.send_message_error(self, self.number, 'Remove liquidity RPCswap', self.address_wallet,
                #                              'Транзакция сфейлилась, пытаюсь еще раз')
                time.sleep(30)
                retry += 1
                if retry > 5:
            ...
            self.log.info(f'[{self.number}] Remove {token_to_remove["name"]} RPCswap || https://nova.arbiscan.io/tx/{Web3.to_hex(tx_hash)}\n')
            # if TgBot.TG_BOT_SEND is True:
            #     TgBot.send_message_success(self, self.number, f'Remove {token_to_remove["name"]} RPCswap', self.address_wallet,
            #                                f'https://nova.arbiscan.io/tx/{Web3.to_hex(tx_hash)}')
