...
class ArbSwap(Approve):
    ...
    def __init__(self, private_key, web3, number, log):
        ...
        self.address = Web3.to_checksum_address('0xee01c0cd76354c383b8c7b4e65ea88d00b06f36f')
    ...
    def buy_token(self, token_to_buy, prescale, retry=0):
        try:
            self.log.info(f'Buy {token_to_buy["name"]} token on ArbSwap')
            ...
            if tx_receipt.status == 1:
                ...
                #     TgBot.send_message_error(self, self.number, 'Buy token ArbSwap', self.address_wallet,
            ...
            self.log.info(f'[{self.number}] Buy {min_tok} {token_to_buy["name"]} ArbSwap || https://nova.arbiscan.io/tx/{Web3.to_hex(tx_hash)}\n')
            # if TgBot.TG_BOT_SEND is True:
            #     TgBot.send_message_success(self, self.number, f'Buy {min_tok} {token_to_buy["name"]} ArbSwap', self.address_wallet,
    ...
    def sold_token(self, token_to_sold, retry=0):
        try:
            self.log.info(f'Sold {token_to_sold["name"]} token on ArbSwap')
            ...
            if tx_receipt.status == 1:
                ...
                self.log.error('Транзакция сфейлилась, пытаюсь еще раз')
                # if TgBot.TG_BOT_SEND is True:
                #     TgBot.send_message_error(self, self.number, 'Sold token ArbSwap', self.address_wallet,
                #                              'Транзакция сфейлилась, пытаюсь еще раз')
                time.sleep(30)
                retry += 1
                if retry > 5:
            ...
            self.log.info(f'[{self.number}] Sold {min_tok} {token_to_sold["name"]} ArbSwap || https://nova.arbiscan.io/tx/{Web3.to_hex(tx_hash)}\n')
            # if TgBot.TG_BOT_SEND is True:
            #     TgBot.send_message_success(self, self.number, f'Sold {min_tok} {token_to_sold["name"]} ArbSwap', self.address_wallet,
            #                                f'https://nova.arbiscan.io/tx/{Web3.to_hex(tx_hash)}')
    ...
    def add_liquidity(self, token_to_add, retry=0):
        try:
            self.log.info(f'Add Liquidity {token_to_add["name"]} token on ArbSwap')
            ...
            if tx_receipt.status == 1:
                ...
                self.log.error('Транзакция сфейлилась, пытаюсь еще раз')
                # if TgBot.TG_BOT_SEND is True:
                #     TgBot.send_message_error(self, self.number, 'Add liquidity ArbSwap', self.address_wallet,
                #                              'Транзакция сфейлилась, пытаюсь еще раз')
                time.sleep(30)
                retry += 1
                if retry > 5:
            ...
            self.log.info(f'[{self.number}] Add {min_tok} {token_to_add["name"]} ArbSwap || https://nova.arbiscan.io/tx/{Web3.to_hex(tx_hash)}\n')
            # if TgBot.TG_BOT_SEND is True:
            #     TgBot.send_message_success(self, self.number, f'Add {min_tok} {token_to_add["name"]} ArbSwap', self.address_wallet,
            #                                f'https://nova.arbiscan.io/tx/{Web3.to_hex(tx_hash)}')
    ...
    def remove_liquidity(self, token_to_remove, retry=0):
        try:
            self.log.info(f'Remove Liquidity {token_to_remove["name"]} token on ArbSwap')

            token_contract = self.web3.eth.contract(address=token_to_remove['lp arbswap'], abi=self.token_remove_abi)
            token_balance = token_contract.functions.balanceOf(self.address_wallet).call()
            if token_balance == 0:
            ...
            if allowance < 10000 * 10 ** decimal:
                self.log.info('Нужен аппрув, делаю')
                self.approve(token_to_remove['lp arbswap'], self.address)
                time.sleep(60)
            ...
            if tx_receipt.status == 1:
                ...
                self.log.error('Транзакция сфейлилась, пытаюсь еще раз')
                # if TgBot.TG_BOT_SEND is True:
                #     TgBot.send_message_error(self, self.number, 'Remove Liquidity ArbSwap', self.address_wallet,
                #                              'Транзакция сфейлилась, пытаюсь еще раз')
                time.sleep(30)
                retry += 1
                if retry > 5:
            ...
            self.log.info(f'[{self.number}] Remove {token_to_remove["name"]} ArbSwap || https://nova.arbiscan.io/tx/{Web3.to_hex(tx_hash)}\n')
            # if TgBot.TG_BOT_SEND is True:
            #     TgBot.send_message_success(self, self.number, f'Remove {token_to_remove["name"]} ArbSwap', self.address_wallet,
            #                                f'https://nova.arbiscan.io/tx/{Web3.to_hex(tx_hash)}')
